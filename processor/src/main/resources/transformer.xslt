...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  -->
	...
	<xsl:template match="event-definition" mode="code">
            public void on<xsl:value-of select="translate(normalize-space(event-type-name),' .','')" /> (<xsl:value-of
            select="translate(normalize-space(event-class-name),' ','')"
        /> event, ActivityContextInterface aci,EventContext context){
              tracer.info("event <xsl:value-of select="normalize-space(event-type-name)" />" );
            }

	</xsl:template>

  	<xsl:template match="event-definition" mode="code-ra"> eventType<xsl:value-of
            select="translate(normalize-space(event-type-name),' .','')"
        /> = super.raContext.getEventLookupFacility().getFireableEventType(new EventTypeID("<xsl:value-of
            select="normalize-space(event-type-name)"
        />" ,super.raContext.getResourceAdaptorTypes()[0].getVendor(), super.raContext.getResourceAdaptorTypes()[0].getVersion())); </xsl:template>

  	<xsl:template match="event-definition" mode="code-ra2"> eventType<xsl:value-of
            select="translate(normalize-space(event-type-name),' .','')"
        />.getEventType(), </xsl:template>
